...
const PARAMS = [
  {
    ...
    label: "Columns",
    type: "int",
    description: "Blocks across. The grid is counts, so it survives a resolution change.",
    animatable: false,
  ...
  },
  {
    ...
    label: "Rows",
    type: "int",
    description: "Blocks down. Separate from columns so blocks can be strips.",
    animatable: false,
    legal: [1, MAX_GRID_AXIS],
    default: 16,
    surprise: {
  ...
  },
  {
    ...
    description: "Fraction of block pairs that exchange places. 0 leaves the image alone.",
    animatable: true,
    legal: [0, 1],
    default: 0.5,
    step: 0.01,
  ...
  },
  {
    ...
    label: "Seed",
    type: "seed",
    description: "Fixes the permutation. The same seed and grid always shuffle the same way.",
    animatable: false,
    default: 0,
    surprise: {
...
const DESCRIPTOR = defineEffect({
  ...
  name: "Block shuffle",
  summary:
    "Chops the picture into a grid of blocks and swaps pairs of them around.",
  description:
    "The rearrangement is a real permutation rather than a hash, so no block arrives twice and none is lost; it is generated by a small cipher, which is what lets every pixel work out where its block went with no CPU round trip. The grid is a count of blocks across and down rather than a block size in pixels, deliberately: a pixel size would make the permutation depend on the working resolution, so the preview and the export would be two different pictures instead of one picture at two sizes. Swap probability sets how much of the image moves.",
  keywords: ["block", "blocks", "shuffle", "swap", "scramble", "mosaic", "tiles", "permute", "rearrange", "corrupt", "puzzle", "jumble"],
  concept: "glitch",
  requirement: "F-GL-12",
