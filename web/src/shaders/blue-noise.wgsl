// F-OD-05 — Blue-noise ordered dither, void-and-cluster tile.
//
// Same machinery as the Bayer tiles and a completely different look. A Bayer
// matrix has energy at low spatial frequencies, which is what makes its
// cross-hatch visible; a void-and-cluster tile has almost none, so the result
// reads as grain rather than as a pattern and survives scaling and resampling
// that would alias a Bayer screen.
//
// 64x64 because the tile has to be large enough that its own period is not
// itself a low-frequency artefact — the thing the algorithm exists to avoid.
// That is 4096 ranks, one 16 KB table, uploaded once.
//
// The tile arrives as a table of integer ranks generated in Rust
// (core/crates/dither-core/src/noise.rs), from an explicit seed so the same
// document reproduces the same grain — see web/src/gpu/matrices.ts.
//
// Conventions: web/src/shaders/CONVENTIONS.md.

const TILE      : i32 = 64;
const TILE_AREA : f32 = 4096.0;
